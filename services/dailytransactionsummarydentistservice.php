...
$dentist =urldecode($_POST['sdentist']);
$result = $service->loadClientTreatment($fromdate, $group, $dentist);
...
class ServiceClass
{
    ...
    public function loadClientTreatment($fromdate, $group, $dentist)
    {
        $key = 'tsoa.soaid';
        if ($group == 'Dentist') {
            $key = 'tsoa.dentist';
        ...
        $query0 = "SELECT result FROM (SELECT DISTINCT $key AS result FROM clientprofile cp INNER JOIN treatmentsub tsub ON tsub.clientid = cp.clientid INNER JOIN treatmentsoa tsoa ON tsoa.soaid = tsub.soaid WHERE tsoa.date = :a AND tsoa.dentist = :b
        UNION 
...
';
    ...
    <hr>
     <div style="display: flex; justify-content: space-between;">
        ...
        <span><strong>₱' . number_format($grandAccumulatedPayments - $totalExpenses, 2) . '</strong></span>
    </div>
</div>
';
    }
